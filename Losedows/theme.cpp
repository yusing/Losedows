...
void apply_default_theme()
{
    ...
    style.Colors[ImGuiCol_CheckMark] = rgba(255, 144, 144, 255);
...
bool load_theme(const char* file)
{
    ...
    if (error != 0 || theme_file == nullptr){
        log("Theme Error: failed to open theme file \"%s\" for read, reason: \"%s\"", file, errno_string(error).c_str());
        return false;
    }
    auto& colors = ImGui::GetStyle().Colors;
    fread(colors, sizeof colors, 1, theme_file);
    if ((error = ferror(theme_file)) != 0){
        log("File I/O Error: failed to read file \"%s\", reason: \"%s\"", file, errno_string(error).c_str());
    }
    else{
...
{
    const auto path = fs::current_path().string() + "\\current.tm";
    return file_exists(path) && load_theme(path.c_str());
}
